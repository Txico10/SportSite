<?php
/** 
 * Livewire permission controller
 * 
 * PHP version 7.4
 * 
 * @category MyCategory
 * @package  MyPackage
 * @author   Stefan Monteiro <stefanmonteiro@gmail.com>
 * @license  MIT treino.localhost
 * @link     link()
 * */
namespace App\Http\Livewire\Admin;
use App\Models\Permission;
use Livewire\Component;
use Livewire\WithPagination;

/**
 *  Livewire permission component
 * 
 * @category MyCategory
 * @package  MyPackage
 * @author   Stefan Monteiro <stefanmonteiro@gmail.com>
 * @license  MIT treino.localhost
 * @link     link()
 * */
class Permissions extends Component
{
    use WithPagination;

    public $perPage = 10;
    public $sortField= 'display_name';
    public $sortAsc = true;
    public $search_permission = '';


    protected $queryString = ['search_permission'=> ['except' => '']];

    protected $listeners =  ['refreshParent' => '$refresh', 
                             'deletePermission'=>'delete',
                             'infoPermission' => 'infoperm'
                            ];

    /**
     * Show the list of permissions
     * 
     * @return view|permissions
     */
    public function render()
    {
        return view(
            'livewire.admin.permissions',
            ['permissions' => Permission::search($this->search_permission)
            ->orderBy($this->sortField, $this->sortAsc ? 'asc' : 'desc')
            ->paginate($this->perPage),
            ]
        );
    }

    /**
     * Render the livewire users view
     * 
     * @param $field represents the field to sort
     * 
     * @return null
     */
    public function sortBy($field)
    {
        if ($this->sortField === $field) {
            $this->sortAsc = ! $this->sortAsc;
        } else {
            $this->sortAsc = true;
        }

        $this->sortField = $field;
    }

    /**
     * Personalize pagination
     * 
     * @return pagination-links
     */
    public function paginationView() 
    {
        return '.includes.pagination-links';
    }

    /**
     * Reset pagination on search
     * 
     * @return resetPage
     */
    public function updatingSearch()
    {
        $this->resetPage();
    }

    /**
     * Personalize pagination
     * 
     * @param $id role id
     * 
     * @return void
     */
    public function delete($id)
    {
        $permission=Permission::findOrFail($id);

        $users = $permission->users;

        if ($users->count()>0) {
            foreach ($users as $user) {
                $user->detachPermission($permission);
            }
        }

        $roles = $permission->roles;

        if ($roles->count()>0) {            
            foreach ($roles as $role) {
                $role->detachPermission($permission);
            }
        }

        $permission->delete();
        
        $this->emit(
            'alert', 
            [
                'type'=>'success',
                'message'=>'Permission deleted successfully'
            ]
        );

    }

    /**
     * Personalize info permission
     * 
     * @param $id permission id
     * 
     * @return void
     */
    public function infoperm($id) 
    {
        $permission = Permission::findOrFail($id);

        $html = '<div class="container-fluid">
                <div class="card card-outline card-primary">
                    <div class="card-header border-0">
        <h3 class="card-title">Showing permission '.$permission->display_name.'</h3>
                        <div class="card-tools">
                            <a href="#" class="btn btn-tool btn-sm">
                                <i class="fas fa-download"></i>
                            </a>
                            <a href="#" class="btn btn-tool btn-sm">
                                <i class="fas fa-bars"></i>
                            </a>
                        </div>
                        </div>
                        <div class="card-body table-responsive p-0">
                            <table class="table table-striped table-valign-middle">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td><p class="text-primary">Role ID</p></td>
                                        <td align="right">'.$permission->id.'</td>
                                    </tr>
                                    <tr>
                                        <td><p class="text-primary">Role tag</p></td>
                                        <td align="right">'.$permission->name.'</td>
                                    </tr>
                                    <tr>
                                <td><p class="text-primary">Role description</p></td>
                                <td align="right">'.$permission->description.'</td>
                                    </tr>
                                    <tr>
                                    <td><p class="text-primary">Permissions</p></td>
                                        <td align="right">';
                                        
        foreach ($permission->roles->chunk(3) as $roles) {
            foreach ($roles as $key => $role) {
                $html = $html.'<span class="badge bg-secondary">'.
                $role->display_name.'</span> &nbsp;';
            }
            $html = $html.'<br>';
        }
        $html = $html.'</td>
                    </tr>
                    <tr>
                        <td><p class="text-primary">Role users</p></td>
                        <td align="right">'.$permission->users->count().'</td>
                    </tr>
                    <tr>
                        <td><p class="text-primary">Created at</p></td>
                        <td align="right">'.$permission->created_at.'</td>
                    </tr>
                    <tr>
                        <td><p class="text-primary">Updated at</p></td>
                        <td align="right">'.$permission->updated_at.'</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>';

        $html = preg_replace('/\>\s+\</m', '><', $html);

        $this->emit('modal-permissionInfo', $html);
        
    }
}
